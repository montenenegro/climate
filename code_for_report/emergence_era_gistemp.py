...
# %% emergence date on raw ERA5 data alone 

def emergence_date_era_alone(sub_df):
    
    sub_df2 = pd.DataFrame(sub_df)
    sub_df2.index = era_mid_time
    
    sub_df_year = sub_df2.resample('Y').mean()
    threshold = np.nanmax(sub_df_year['1979':'2000'])
    
    if np.sum(sub_df_year.iloc[:,0] > threshold) > 0:
        emergence = sub_df_year[sub_df_year.iloc[:,0] > threshold].index[0]
        # plt.figure()
        # plt.plot(sub_df_year)
        # plt.axhline(threshold, LineStyle='--', color='red')
...
era_mid_time = pd.to_datetime(np.array(era_mid['time']), format='%Y-%*-%dT00:00:00.000000000')

start_time = time.time()
start_local_time = time.ctime(start_time)

era_mid_ED = np.apply_along_axis(emergence_date_era_alone, 0, era_mid_temp)
